...
function isStateValid(state: string | undefined): boolean {
	return typeof state === "string" && stateStore.has(state);
}
...
export async function handleAuthCallback(params: {
	...
	expiresAt: string;
	state?: string;
	allowStateBypass?: boolean;
}): Promise<{ success: boolean; error?: string }> {
	if (!isStateValid(params.state)) {
		if (!params.allowStateBypass) {
			return { success: false, error: "Invalid or expired auth session" };
		}
		console.warn(
			"[auth] Accepting callback without matching state (state bypass enabled)",
		);
...
export function parseAuthDeepLink(
	...
	try {
		...
		const token = getSearchParam(parsed.searchParams, [
			"token",
			"authToken",
			"accessToken",
			"access_token",
		]);
		const expiresAt = getSearchParam(parsed.searchParams, [
			"expiresAt",
			"expires_at",
			"expires",
		]);
