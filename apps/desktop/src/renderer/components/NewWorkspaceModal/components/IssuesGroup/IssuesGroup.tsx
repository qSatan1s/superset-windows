...
export function IssuesGroup({ projectId }: IssuesGroupProps) {
	...
	const isLinearConnected =
		integrations?.some((integration) => integration.provider === "linear") ??
		false;
	...
	const slugWidth = useMemo(
		() => getSlugColumnWidth(visibleTasks.map((task) => task.slug)),
		[visibleTasks],
	);
		...
		return (
			<div className="flex flex-col items-center gap-3 py-8 px-4 text-center">
				...
				<div className="space-y-1">
					<p className="text-sm font-medium">{tt("Connect Linear")}</p>
					<p className="text-xs text-muted-foreground">
						{tt("Sync issues from Linear to create workspaces")}
					</p>
				</div>
				<Button
					size="sm"
	...
	return (
		<CommandGroup>
			<CommandEmpty>{tt("No issues found.")}</CommandEmpty>
			{visibleTasks.map((task) => (
				<CommandItem
					key={task.id}
					onSelect={() => {
						if (!projectId) {
							toast.error(tt("Select a project first"));
							return;
						}
