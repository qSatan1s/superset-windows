	...
	return (
		<>
			<div className="flex items-center justify-between border-b px-3 py-2">
				...
				>
					<TabsList>
						<TabsTrigger value="prompt">{tt("Prompt")}</TabsTrigger>
						<TabsTrigger value="issues">{tt("Issues")}</TabsTrigger>
						<TabsTrigger value="pull-requests">
							{tt("Pull requests")}
						</TabsTrigger>
						<TabsTrigger value="branches">{tt("Branches")}</TabsTrigger>
					</TabsList>
				</Tabs>
				<ProjectSelector
			...
			{isListTab ? (
				<Command shouldFilter={false} className={COMMAND_CLASS_NAME}>
					<CommandInput
						...
						placeholder={
							draft.activeTab === "issues"
								? tt("Search by slug, title, or description")
								: draft.activeTab === "branches"
									? tt("Search by name")
									: tt("Search by title, number, author, or paste a url")
						}
					/>
