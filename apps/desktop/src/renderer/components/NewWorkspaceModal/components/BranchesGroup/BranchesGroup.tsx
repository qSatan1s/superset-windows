...
export function BranchesGroup({ projectId }: BranchesGroupProps) {
	...
	const workspaceByBranch = useMemo(() => {
		...
		for (const workspace of allWorkspaces) {
			if (workspace.projectId === projectId) {
				map.set(workspace.branch, workspace.id);
			}
		}
		return map;
	...
	const baseBranchRows = useMemo(
		() => (filterMode === "worktrees" ? worktreeBranchRows : branchRows),
	...
	const handleCreate = useCallback(
		(branchName: string) => {
			...
			void runAsyncAction(
				...
				{
					loading: tt("Creating workspace from branch..."),
		...
		[createBranchWorkspace, projectId, runAsyncAction, tt],
	);
	...
	const handleImportExternalWorktree = useCallback(
		...
		[openExternalWorktree, projectId, runAsyncAction, tt],
	);
	...
	const handleImportAll = useCallback(async () => {
		...
		try {
			...
			toast.success(
				result.imported === 1
					? tt("Imported 1 workspace")
					: tt("Imported {count} workspaces", { count: result.imported }),
			);
		} catch (error) {
			toast.error(
				error instanceof Error
	...
	if (!projectId) {
		return (
			<CommandGroup>
				<CommandEmpty>{tt("Select a project to view branches.")}</CommandEmpty>
			</CommandGroup>
		);
	}
	...
	if (isLocalLoading) {
		return (
			<CommandGroup>
				<CommandEmpty>{tt("Loading branches...")}</CommandEmpty>
			</CommandGroup>
		);
	}
			...
			<CommandGroup>
				<CommandEmpty>
					{filterMode === "worktrees" && isExternalWorktreesLoading
						? tt("Loading worktree branches...")
						: filterMode === "worktrees"
							? tt("No worktree branches found.")
							: tt("No branches found.")}
				</CommandEmpty>
						...
						>
							...
							{existingWorkspaceId ? (
								<span className="shrink-0 hidden group-data-[selected=true]:inline-flex items-center gap-1.5">
									...
									>
										{tt("Duplicate branch")} {modKey}+{tt("Enter")}
									</Button>
								</span>
							) : (
								<Button
