...
export function GitSettings({ visibleItems }: GitSettingsProps) {
	...
	const branchPrefixModeLabels: Record<BranchPrefixMode, string> = {
		none: tt("No prefix"),
		github: tt("GitHub username"),
		author: tt("Git author name"),
		custom: tt("Custom prefix"),
	};

	return (
		<div className="p-6 max-w-4xl w-full">
			<div className="mb-8">
				<h2 className="text-xl font-semibold">{tt("Git & Worktrees")}</h2>
				<p className="text-sm text-muted-foreground mt-1">
					{tt("Configure git branch and worktree behavior")}
				</p>
			</div>
			...
			<div className="space-y-6">
				{showDeleteLocalBranch && (
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							...
							>
								{tt("Delete local branch on workspace removal")}
							</Label>
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-2">
							...
							>
								...
								<SelectContent>
									{(
										Object.entries(branchPrefixModeLabels) as [
											BranchPrefixMode,
											string,
										][]
									).map(([value, label]) => (
				...
				{showWorktreeLocation && (
					<div className="space-y-0.5">
						<Label className="text-sm font-medium">
							{tt("Worktree location")}
						</Label>
						<p className="text-xs text-muted-foreground">
							{tt("Base directory for new worktrees")}
						</p>
						<WorktreeLocationPicker
							currentPath={worktreeBaseDir}
							defaultPathLabel={tt("Default ({path})", {
