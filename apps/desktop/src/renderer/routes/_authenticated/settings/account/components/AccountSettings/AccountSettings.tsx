...
export function AccountSettings({ visibleItems }: AccountSettingsProps) {
	...
	const signOutMutation = electronTrpc.auth.signOut.useMutation({
		onSuccess: () => toast.success(tt("Signed out")),
	});
	...
	async function handleAvatarUpload() {
		...
		try {
			...
			toast.success(tt("Avatar updated!"));
		} catch {
			toast.error(tt("Failed to update avatar"));
		}
	}
	...
	async function handleNameBlur() {
		...
		try {
			...
			toast.success(tt("Name updated!"));
		} catch {
			toast.error(tt("Failed to update name"));
			setNameValue(user.name ?? "");
		}
	}
	...
	return (
		<div className="p-6 max-w-4xl w-full">
			<div className="mb-8">
				<h2 className="text-xl font-semibold">{tt("Account")}</h2>
				<p className="text-sm text-muted-foreground mt-1">
					{tt("Manage your account settings")}
				</p>
			</div>
			...
			<div className="space-y-8">
				{showProfile && (
					<div>
						<h3 className="text-sm font-medium mb-4">{tt("Profile")}</h3>
						{isLoading ? (
							<ProfileSkeleton />
						) : user ? (
							<Card>
										...
										<li className="flex items-center justify-between gap-8 pb-6 border-b border-border">
											<div className="flex-1">
												...
												<div className="text-xs text-muted-foreground">
													{tt("Recommended size is 256x256px")}
												</div>
											</div>
											<button
												type="button"
				...
				{showSignOut && (
					<div className={showProfile ? "pt-6 border-t" : ""}>
						<h3 className="text-sm font-medium mb-2">{tt("Sign Out")}</h3>
						<p className="text-sm text-muted-foreground mb-4">
							{tt("Sign out of your Superset account on this device.")}
						</p>
