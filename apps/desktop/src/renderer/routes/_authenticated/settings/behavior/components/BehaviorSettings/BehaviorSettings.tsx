...
export function BehaviorSettings({ visibleItems }: BehaviorSettingsProps) {
	...
	return (
		<div className="p-6 max-w-4xl w-full">
			<div className="mb-8">
				<h2 className="text-xl font-semibold">{tt("General")}</h2>
				<p className="text-sm text-muted-foreground mt-1">
					{tt("Configure general app preferences")}
				</p>
			</div>
					...
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							<Label htmlFor="confirm-on-quit" className="text-sm font-medium">
								{tt("Confirm before quitting")}
							</Label>
							<p className="text-xs text-muted-foreground">
								{tt("Show a confirmation dialog when quitting the app")}
							</p>
						</div>
						<Switch
							id="confirm-on-quit"
				...
				{showFileOpenMode && (
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							...
							<p className="text-xs text-muted-foreground">
								{tt("Choose how files open when no preview pane exists")}
							</p>
						</div>
						<Select
						...
						>
							...
							<SelectContent>
								<SelectItem value="split-pane">{tt("Split pane")}</SelectItem>
								<SelectItem value="new-tab">{tt("New tab")}</SelectItem>
							</SelectContent>
						</Select>
					</div>
				)}
				...
				{showResourceMonitor && (
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							...
							<p className="text-xs text-muted-foreground">
								{tt("Show CPU and memory usage in the top bar")}
							</p>
						</div>
						<Switch
							id="resource-monitor"
					...
					<div className="flex items-center justify-between">
						<div className="space-y-0.5">
							<Label htmlFor="telemetry" className="text-sm font-medium">
								{tt("Send anonymous usage data")}
							</Label>
							<p className="text-xs text-muted-foreground">
								{tt("Help improve Superset by sending anonymous usage data")}
							</p>
						</div>
						<Switch
							id="telemetry"
