...
export const SETTINGS_ITEMS: SettingsItem[] = [
	...
	},
	{
		...
		section: "appearance",
		title: "Language",
		description: "Choose app interface language",
		keywords: [
			"appearance",
			"language",
			"locale",
	...
	},
	{
		...
		description:
			"Persistent access to Documents, Downloads, and Desktop from terminal sessions",
		keywords: [
			"permissions",
			"full disk access",
			"fda",
...
function buildSearchTokens(
	...
	const localizedKeywords = item.keywords.map((keyword) =>
		translateText(language, keyword),
	);
	...
	// Deduplicate and normalize once to avoid repeated lowercasing per query.
	return [...new Set(allTokens.map((value) => value.toLowerCase()))];
}

function getSearchIndex(language: AppLanguage): SearchIndexItem[] {
	const cached = SEARCH_INDEX_CACHE.get(language);
	if (cached) {
		return cached;
	}

	const index = SETTINGS_ITEMS.map((item) => ({
		id: item.id,
		searchTokens: buildSearchTokens(item, language),
	}));

	SEARCH_INDEX_CACHE.set(language, index);
	return index;
}

function resolveSearchLanguage(language?: AppLanguage): AppLanguage {
...
export function searchSettings(
	...
	const resolvedLanguage = resolveSearchLanguage(language);
	const index = getSearchIndex(resolvedLanguage);
	const q = query.toLowerCase();
	const matchedIds = new Set<SettingItemId>();

	for (const entry of index) {
		if (entry.searchTokens.some((token) => token.includes(q))) {
			matchedIds.add(entry.id);
		}
	}

	return SETTINGS_ITEMS.filter((item) => matchedIds.has(item.id));
}
...
export function getMatchCountBySection(
	...
	const matches = searchSettings(query, language);
