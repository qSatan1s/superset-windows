 ...
 *
 * IPC Protocol:
 * - Uses NDJSON (newline-delimited JSON) over local IPC endpoint
 * - Endpoint: Unix socket (~/.superset/terminal-host.sock) or Windows named pipe
...
function isSocketLive(): Promise<boolean> {
	return new Promise((resolve) => {
		...
		testSocket.connect(SOCKET_ENDPOINT);
	});
}
...
async function startServer(): Promise<void> {
	...
	if (!USE_FILESYSTEM_SOCKET || existsSync(SOCKET_ENDPOINT)) {
		...
		// Named pipes are kernel objects and cannot be removed as files.
		if (USE_FILESYSTEM_SOCKET && existsSync(SOCKET_ENDPOINT)) {
			try {
	...
	await new Promise<void>((resolve, reject) => {
		...
		newServer.listen(SOCKET_ENDPOINT, () => {
			// Set socket permissions (readable/writable by owner only)
			if (USE_FILESYSTEM_SOCKET) {
				try {
					chmodSync(SOCKET_ENDPOINT, 0o600);
				} catch {
			...
			log("info", `Socket: ${SOCKET_ENDPOINT}`);
