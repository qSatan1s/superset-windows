...
const SOCKET_ENDPOINT = getTerminalHostEndpoint(SUPERSET_HOME_DIR);
const USE_FILESYSTEM_SOCKET = usesFilesystemSocket();
...
export class TerminalHostClient extends EventEmitter {
	...
	private async tryConnectControl(): Promise<boolean> {
		return new Promise((resolve) => {
			if (USE_FILESYSTEM_SOCKET) {
				if (!existsSync(SOCKET_ENDPOINT)) {
					resolve(false);
					return;
				}
			} else if (!existsSync(PID_PATH)) {
				// Named pipes aren't visible as files; use PID file as readiness hint.
				resolve(false);
				return;
			}

			const socket = connect(SOCKET_ENDPOINT);
			let resolved = false;
	...
	private async shutdownLegacyDaemon({
		...
		if (USE_FILESYSTEM_SOCKET && !existsSync(SOCKET_ENDPOINT)) return;
		if (!USE_FILESYSTEM_SOCKET && !existsSync(PID_PATH)) return;
	...
	private isSocketLive(): Promise<boolean> {
		return new Promise((resolve) => {
			...
			const testSocket = connect(SOCKET_ENDPOINT);
			const timeout = setTimeout(() => {
				testSocket.destroy();
				resolve(false);
			}, 1000);
	...
	private async spawnDaemon(): Promise<void> {
		...
		if (!USE_FILESYSTEM_SOCKET || existsSync(SOCKET_ENDPOINT)) {
			...
			// Windows named pipes are kernel objects and cannot be unlinked as files.
			if (USE_FILESYSTEM_SOCKET && existsSync(SOCKET_ENDPOINT)) {
				if (DEBUG_CLIENT) {
					console.log("[TerminalHostClient] Removing stale socket file");
				}
				try {
					unlinkSync(SOCKET_ENDPOINT);
				} catch {
	...
	private async waitForDaemon(): Promise<void> {
		...
		while (Date.now() - startTime < SPAWN_WAIT_MS) {
			if (USE_FILESYSTEM_SOCKET) {
				...
				// Named pipe readiness is validated with an active connect attempt.
				const live = await this.isSocketLive();
				if (live) return;
			}
			await this.sleep(100);
