	...
	it("replaces stale Cursor hook commands from old superset paths", () => {
		...
		const currentHookPath =
			process.platform === "win32"
				? "/tmp/.superset-new/hooks/cursor-hook.ps1"
				: "/tmp/.superset-new/hooks/cursor-hook.sh";
		...
		const expectedCommand =
			process.platform === "win32"
				? `powershell -ExecutionPolicy Bypass -NoProfile -NonInteractive -File "${currentHookPath}" Start`
				: `${currentHookPath} Start`;

		expect(
			beforeSubmitPrompt.some((entry) => entry.command === expectedCommand),
		).toBe(true);
		expect(
