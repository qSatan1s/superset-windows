...
export const CURSOR_HOOK_PS1_SCRIPT_NAME = "cursor-hook.ps1";
...
const CURSOR_HOOK_PS1_TEMPLATE_PATH = path.join(
	__dirname,
	"templates",
	"cursor-hook.template.ps1",
);
...
export function getCursorHookPs1ScriptPath(): string {
	return path.join(HOOKS_DIR, CURSOR_HOOK_PS1_SCRIPT_NAME);
}
...
function getCursorHookPs1Content(): string {
	const template = fs.readFileSync(CURSOR_HOOK_PS1_TEMPLATE_PATH, "utf-8");
	return template
		.replace("{{MARKER}}", CURSOR_HOOK_MARKER)
		.replace(/\{\{DEFAULT_PORT\}\}/g, String(env.DESKTOP_NOTIFICATIONS_PORT));
}

function buildHookCommand(scriptPath: string, eventArg: string): string {
	if (IS_WINDOWS) {
		return `powershell -ExecutionPolicy Bypass -NoProfile -NonInteractive -File "${scriptPath}" ${eventArg}`;
	}
	return `${scriptPath} ${eventArg}`;
}

function isCursorHookManaged(entry: CursorHookEntry): boolean {
	const cmd = entry.command;
	if (!cmd) return false;
	const normalized = cmd.replaceAll("\\", "/");
	return (
		normalized.includes("/hooks/cursor-hook.sh") ||
		normalized.includes("/hooks/cursor-hook.ps1") ||
		isSupersetManagedHookCommand(cmd, CURSOR_HOOK_SCRIPT_NAME) ||
		isSupersetManagedHookCommand(cmd, CURSOR_HOOK_PS1_SCRIPT_NAME)
	);
}
...
export function getCursorHooksJsonContent(hookScriptPath: string): string {
	...
	const ourHooks: Record<string, CursorHookEntry> = {
		beforeSubmitPrompt: {
			command: buildHookCommand(hookScriptPath, "Start"),
		},
...
export function createCursorHookScript(): void {
	...
	console.log(
		`[agent-setup] ${changed ? "Updated" : "Verified"} Cursor hook script (.sh)`,
	);

	if (IS_WINDOWS) {
		const ps1Path = getCursorHookPs1ScriptPath();
		const ps1Content = getCursorHookPs1Content();
		const ps1Changed = writeFileIfChanged(ps1Path, ps1Content, 0o644);
		console.log(
			`[agent-setup] ${ps1Changed ? "Updated" : "Verified"} Cursor hook script (.ps1)`,
		);
	}
}
