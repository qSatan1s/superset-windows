...
// OAuth callback fallback for Windows/Linux and development environments where
// custom URI handlers can be unreliable. Browser can hit localhost directly to
// complete sign-in.
app.get("/auth/callback", async (req, res) => {
	...
	const expiresAt =
		req.query.expiresAt ?? req.query.expires_at ?? req.query.expires;
	const state = req.query.state;

	if (typeof token !== "string" || typeof expiresAt !== "string") {
		return res
			.status(400)
			.json({ success: false, error: "Missing auth token/expiry params" });
	}
	...
	const result = await handleAuthCallback({
		...
		// Some Windows browser flows drop/replace state during localhost callback.
		allowStateBypass: process.platform === "win32",
	});
	if (!result.success) {
		// Final fallback for Windows localhost callback: persist token directly.
