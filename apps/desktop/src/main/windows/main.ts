...
export async function MainWindow() {
	...
	window.once("ready-to-show", showAndFocusWindow);

	let startupShowFallback: ReturnType<typeof setTimeout> | null = setTimeout(
		() => {
			if (window.isDestroyed() || window.isVisible()) return;
			console.warn(
				"[main-window] Startup fallback triggered, forcing window visibility",
			);
			showAndFocusWindow();
		},
		7000,
	);
	startupShowFallback.unref?.();
	...
	window.webContents.once("did-finish-load", async () => {
		...
		showAndFocusWindow();
		if (startupShowFallback) {
			clearTimeout(startupShowFallback);
			startupShowFallback = null;
		}
		initialized = true;
	});
